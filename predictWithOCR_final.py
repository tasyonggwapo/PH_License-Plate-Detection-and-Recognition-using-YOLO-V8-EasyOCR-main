...
# Set to track unique detections
detected_plates = set()
    ...
    def write_results(self, idx, preds, batch):
        ...
        for *xyxy, conf, cls in reversed(det):
            ...
            if self.args.save or self.args.save_crop or self.args.show:
                ...
                # Use OCR on detected bounding box
                ocr_text = getOCR(im0, xyxy)
                if ocr_text and ocr_text not in detected_plates:  # Check for unique detection
                    label = ocr_text
                    # Append OCR result to the output file if it's a new detection
                    with open(output_file, 'a') as text_file:
                        text_file.write(ocr_text + '\n')
                    detected_plates.add(ocr_text)  # Add to set of detected plates
